...
| `DecoyMinSec` / `DecoyMaxSec` | int | 10 / 30 | 위장 이동 간격(초) 랜덤 범위 |
...
## 위장 모드 (테스트 기능, `Native` + `TrayApp`)
정식 측정 기능이 아니라, **`GetLastInputInfo`가 하드웨어/합성 입력을 구분하지 못한다**는 한계를 직접 보여주기 위한 실험 기능. 합성 입력으로 유휴를 0으로 유지하면 외부 idle-reader는 물론 **이 앱의 측정값도 함께 오염**된다(의도된 데모).
...
- **토글/안전장치**: 트레이 메뉴 `위장 모드 (테스트 기능)`. 켤 때 `ToggleDecoy`가 **책임 고지 + 테스트 기능 명시 팝업**(기본 No)을 띄우고 동의 시에만 `StartDecoy`. `config.ini`의 `DecoyEnabled=true`면 시작 시 팝업 없이 자동 활성(명시적 설정으로 간주). 켜진 상태는 툴팁 `[위장]` + 켜짐 풍선으로 표시. 설정 창(`SettingsForm`)에는 노출하지 않음(실험 기능).
