...
**첫 실행 시** ① 면책 조항 동의 창 → ② 근무시간 설정 창이 차례로 표시됩니다.
- 면책 조항에 동의해야 실행되며, 동의하지 않으면 시작되지 않습니다. (동의 기록은 `%APPDATA%\IdleTimer\consent.txt`)
- 근무시간 설정은 근무 시작/종료·표준 근무시간·야간 시간대 등 핵심 값을 미리 정합니다. **"기본값으로 시작"** 으로 건너뛰면 기본값(09:00–18:00 등)이 적용됩니다.
- 설정은 이후 트레이 메뉴 **설정…** 에서 언제든 다시 바꿀 수 있습니다.

트레이에 파란 시계 아이콘이 나타납니다.
- **더블클릭** → 오늘 현황
- **우클릭 메뉴**: 오늘 현황 / 주간 리포트 / 주간 히트맵 / 일시정지 / 시작 시 자동 실행 / **설정…** / 데이터·설정 폴더 열기 / 설정 다시 읽기 / **도움말** / 종료
...
각 알림은 설정 창에서 개별로 켜고 끌 수 있고, **알림 사용 (전체)** 마스터 토글로 모든 알림을 한 번에 끌 수 있습니다.
...
NotifyEnabled=true          # 전체 알림 마스터 (false면 아래 개별값과 무관하게 모든 알림 끔)
NotifyClockOut=true
NotifyNight=true
NotifyBreak=true
NotifyOvertime=true
